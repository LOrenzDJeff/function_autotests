...
@allure.story('21.1.1:Базовая проверка функционала ECMP')
...
def test_base_function_ECMP_part21_1(DUT):
    resp = ''
    conn = Telnet()
    ...
    if DUT.host_ip == DUT1.host_ip:
        ...
        isis_neighbors = [
            ['0010.0000.0002', 'te0/0/1', 'up', 'level-2', 'E0D9.E3FF.4887', '', 'true', 'none', DUT2.hostname],
            ['0010.0000.0002', 'te0/0/2', 'up', 'level-2', 'E0D9.E3FF.4888', '', 'true', 'none', DUT2.hostname],
            ['0010.0000.0001', 'te0/0/3', 'up', 'level-2', 'A8F9.4B8B.92AD', '', 'true', 'none', DUT3.hostname],
            ['0010.0000.0001', 'te0/0/4', 'up', 'level-2', 'A8F9.4B8B.92AE', '', 'true', 'none', DUT3.hostname]
        ]
        for i in range(4):
            assert_that(result[i]['nbr_system_id'] == isis_neighbors[i][0], 'Системный id равен %s, а должен быть %s'%(result[0]['nbr_system_id'],isis_neighbors[i][0]))
            assert_that(result[i]['nbr_int'] == isis_neighbors[i][1], 'Интерфейс равен %s, а должен быть %s'%(result[0]['nbr_int'],isis_neighbors[i][1]))
            assert_that(result[i]['nbr_state'] == isis_neighbors[i][2], 'Статус равен %s, а должен быть %s'%(result[0]['nbr_state'],isis_neighbors[i][2]))
            assert_that(result[i]['nbr_type'] == isis_neighbors[i][3], 'Тип равен %s, а должен быть %s'%(result[0]['nbr_type'],isis_neighbors[i][3]))
            assert_that(result[i]['nbr_hostname'] == isis_neighbors[i][8], 'Хост равен %s, а должен быть %s'%(result[0]['nbr_hostname'],isis_neighbors[i][8]))
        ...
        route = [
            ['192.168.73.0/24','192.168.3.18','te0/0/1','isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.73.0/24','192.168.3.22',"te0/0/2",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.73.0/24','192.168.2.10',"te0/0/3",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.73.0/24','192.168.2.13',"te0/0/4",'isis','116','20','isis-level2-internal','none','remote']
        ]
        l3forwarding = [
        ...
        assert_that(result['type'] == route[j][6], 'Тип маршрутизации указан %s, а должен быть %s'%(result['route_type'],route[j][6]))
        assert_that(result['protect'] == route[j][7], 'Тип указан %s, а должен быть %s'%(result['type'],route[j][7]))
        assert_that(result['route_type'] == route[j][8], 'Защита указана %s, а должна быть %s'%(result['protect'],route[j][8]))
        ...
        for i in range(4):
            ...
            assert_that(result[i]['type'] == route[j][6], 'Тип маршрутизации указан %s, а должен быть %s'%(result[i]['route_type'],route[j][6]))
            assert_that(result[i]['protect'] == route[j][7], 'Тип указан %s, а должен быть %s'%(result[i]['type'],route[j][7]))
            assert_that(result[i]['route_type'] == route[j][8], 'Защита указана %s, а должна быть %s'%(result[i]['protect'],route[j][8]))
    ...
    if DUT.host_ip == DUT2.host_ip:
        ...
        isis_neighbors = [
            ['0010.0000.0003', 'te0/0/1', 'up', 'level-2', 'E45A.D4DE.C881', '', 'true', 'none', DUT1.hostname],
            ['0010.0000.0003', 'te0/0/2', 'up', 'level-2', 'E45A.D4DE.C882', '', 'true', 'none', DUT1.hostname],
            ['0010.0000.0001', 'te0/0/3', 'up', 'level-2', 'A8F9.4B8B.9383', '', 'true', 'none', DUT3.hostname],
            ['0010.0000.0001', 'te0/0/4', 'up', 'level-2', 'A8F9.4B8B.9384', '', 'true', 'none', DUT3.hostname]
        ]
        for i in range(4):
            assert_that(result[i]['nbr_system_id'] == isis_neighbors[i][0], 'Системный id равен %s, а должен быть %s'%(result[0]['nbr_system_id'],isis_neighbors[i][0]))
            assert_that(result[i]['nbr_int'] == isis_neighbors[i][1], 'Интерфейс равен %s, а должен быть %s'%(result[0]['nbr_int'],isis_neighbors[i][1]))
        ...
        route = [
            ['192.168.70.0/24','192.168.1.13',"te0/0/3",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.70.0/24','192.168.1.10',"te0/0/4",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.70.0/24','192.168.3.17','te0/0/1','isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.70.0/24','192.168.3.21',"te0/0/2",'isis','116','20','isis-level2-internal','none','remote']
        ]
        l3forwarding = [
    ...
    if DUT.host_ip == DUT3.host_ip:
        ...
        isis_neighbors = [
            ['0010.0000.0003', 'te0/0/1', 'up', 'level-2', 'E45A.D4DE.C883', '', 'true', 'none', DUT1.hostname],
            ['0010.0000.0003', 'te0/0/2', 'up', 'level-2', 'E45A.D4DE.C884', '', 'true', 'none', DUT1.hostname],
            ['0010.0000.0002', 'te0/0/3', 'up', 'level-2', 'E0D9.E3FF.4883', '', 'true', 'none', DUT2.hostname],
            ['0010.0000.0002', 'te0/0/4', 'up', 'level-2', 'E0D9.E3FF.4884', '', 'true', 'none', DUT2.hostname]
        ]
        for i in range(4):
            assert_that(result[i]['nbr_system_id'] == isis_neighbors[i][0], 'Системный id равен %s, а должен быть %s'%(result[0]['nbr_system_id'],isis_neighbors[i][0]))
            assert_that(result[i]['nbr_int'] == isis_neighbors[i][1], 'Интерфейс равен %s, а должен быть %s'%(result[0]['nbr_int'],isis_neighbors[i][1]))
        ...
        route = [
            ['192.168.74.0/24','192.168.1.14',"te0/0/3",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.74.0/24','192.168.1.9',"te0/0/4",'isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.74.0/24','192.168.2.9','te0/0/1','isis','116','20','isis-level2-internal','none','remote'],
            ['192.168.74.0/24','192.168.2.14',"te0/0/2",'isis','116','20','isis-level2-internal','none','remote']
        ]
        l3forwarding = [
