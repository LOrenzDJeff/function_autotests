...
@pytest.mark.init_config21_2
@pytest.mark.parametrize("DUT",
			[
			 pytest.param(DUT1, marks=pytest.mark.dependency(name="load_config212_dut1",scope="session")), 
 			 pytest.param(DUT2, marks=pytest.mark.dependency(name="load_config212_dut2",scope="session")), 
 			 pytest.param(DUT3, marks=pytest.mark.dependency(name="load_config212_dut3",scope="session"))
			]
			)
        
def test_me_init_config_upload_part21_2(DUT):
# В данном тесте будем загружать начальную конфигурацию на ME маршрутизаторы для тестов из Части 21.1 документа   
   DUT.connection()
   DUT.startup()
