...
def remove_undo_test(DUT):
  ...
  if DUT.host_ip == DUT1.host_ip:
      ...
      conn.execute("load-interval 20")
      conn.execute("router ospfv2 test")
      conn.execute("area 0.0.0.0")
      conn.execute("interface te0/0/3")
      conn.execute("metric 10")
      conn.execute("commit")
      conn.execute("end")
      ...
      conn.execute("network point-to-point")
...
@allure.story('21.2.3:Тестирование функционала балансировки IGP: OSPF трафика между интерфейсами методом лестница')
@allure.title('Проверка балансировки IGP: OSPF трафика между интерфейсами с отключением интерфейсов')
...
def test_ECMP_ospf_upstair_21_2(DUT, ip_iperf_server, vrf_cli, vrf_cli2):
  ...
  for i in range(1, step + 1):
    ...
    if i != step:
      ...
      for j in range(step - i):
        ...
        assert_that(int(processed_result[j]['Int_send'])>10,"%s: На интерфейсе %s отсутствует исходящий трафик тест закончился на этапе %d"%(DUT.hostname,processed_result[j]['Int_name'], i))
      delta_send = []
      sum = 0
      ...
      for j in range(len(delta_send)):
        assert_that(delta_send[j]<=50,"%s: Балансировка IP трафика в %s очень плохая т.к. отклонение равно или превысило 50%% и равно - %d%% тест закончился на этапе %d"%(DUT.hostname,mas[j * 2],delta_send[j], i))
        assert_that(delta_send[j]<=40,"%s: Балансировка IP трафика в %s плохая т.к. отклонение равно или превысило 40%% и равно - %d%% тест закончился на этапе %d"%(DUT.hostname,mas[j * 2],delta_send[j], i))
        #assert_that(delta_send[j]<=30,"%s: Балансировка IP трафика в %s средняя т.к. отклонение равно или превысило 30%% и равно - %d%% тест закончился на этапе %d"%(DUT.hostname,mas[j * 2],delta_send[j], i))
        #assert_that(delta_send[j]<=20,"%s: Балансировка IP трафика в %s нормальная т.к. отклонение равно или превысило 20%% и равно - %d%% тест закончился на этапе %d"%(DUT.hostname,mas[j * 2],delta_send[j], i))
        #assert_that(delta_send[j]<=15,"%s: Балансировка IP трафика в %s приемлемая т.к. отклонение равно или превысило 15%% и равно - %d%% тест закончился на этапе %d"%(DUT.hostname,mas[j * 2],delta_send[j], i))
        if delta_send[j]>=30:
      ...
      assert_that(int(int1_send)>10,"%s: На интерфейсе %s отсутствует исходящий трафик тест закончился на этапе 4"%(DUT.hostname,int1_name))
      assert_that(int(int2_send)>10,"%s: На интерфейсе %s отсутствует исходящий трафик тест закончился на этапе 4"%(DUT.hostname,int2_name))
      assert_that(int(int3_send)>10,"%s: На интерфейсе %s отсутствует исходящий трафик тест закончился на этапе 4"%(DUT.hostname,int3_name))
      assert_that(int(int4_send)>10,"%s: На интерфейсе %s отсутствует исходящий трафик тест закончился на этапе 4"%(DUT.hostname,int4_name))
      ...
      assert_that(delta1_send<=50,"%s: Балансировка IP трафика в %s очень плохая т.к. отклонение равно или превысило 50%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int1_name,delta1_send))
      assert_that(delta1_send<=40,"%s: Балансировка IP трафика в %s плохая т.к. отклонение равно или превысило 40%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int1_name,delta1_send))
      #assert_that(delta1_send<=30,"%s: Балансировка IP трафика в %s средняя т.к. отклонение равно или превысило 30%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int1_name,delta1_send))
      #assert_that(delta1_send<=20,"%s: Балансировка IP трафика в %s нормальная т.к. отклонение равно или превысило 20%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int1_name,delta1_send))
      #assert_that(delta1_send<=15,"%s: Балансировка IP трафика в %s приемлемая т.к. отклонение равно или превысило 15%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int1_name,delta1_send))

      assert_that(delta2_send<=50,"%s: Балансировка IP трафика в %s очень плохая т.к. отклонение равно или превысило 50%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int2_name,delta2_send))
      assert_that(delta2_send<=40,"%s: Балансировка IP трафика в %s плохая т.к. отклонение равно или превысило 40%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int2_name,delta2_send))
      #assert_that(delta2_send<=30,"%s: Балансировка IP трафика в %s средняя т.к. отклонение равно или превысило 30%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int2_name,delta2_send))
      #assert_that(delta2_send<=20,"%s: Балансировка IP трафика в %s нормальная т.к. отклонение равно или превысило 20%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int2_name,delta2_send))
      #assert_that(delta2_send<=15,"%s: Балансировка IP трафика в %s приемлемая т.к. отклонение равно или превысило 15%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int2_name,delta2_send))

      assert_that(delta3_send<=50,"%s: Балансировка IP трафика в %s очень плохая т.к. отклонение равно или превысило 50%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int3_name,delta3_send))
      assert_that(delta3_send<=40,"%s: Балансировка IP трафика в %s плохая т.к. отклонение равно или превысило 40%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int3_name,delta3_send))
      #assert_that(delta3_send<=30,"%s: Балансировка IP трафика в %s средняя т.к. отклонение равно или превысило 30%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int3_name,delta3_send))
      #assert_that(delta3_send<=20,"%s: Балансировка IP трафика в %s нормальная т.к. отклонение равно или превысило 20%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int3_name,delta3_send))
      #assert_that(delta3_send<=15,"%s: Балансировка IP трафика в %s приемлемая т.к. отклонение равно или превысило 15%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int3_name,delta3_send))

      assert_that(delta4_send<=50,"%s: Балансировка IP трафика в %s очень плохая т.к. отклонение равно или превысило 50%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int4_name,delta4_send))
      assert_that(delta4_send<=40,"%s: Балансировка IP трафика в %s плохая т.к. отклонение равно или превысило 40%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int4_name,delta4_send))
      #assert_that(delta4_send<=30,"%s: Балансировка IP трафика в %s средняя т.к. отклонение равно или превысило 30%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int4_name,delta4_send))
      #assert_that(delta4_send<=20,"%s: Балансировка IP трафика в %s нормальная т.к. отклонение равно или превысило 20%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int4_name,delta4_send))
      #assert_that(delta4_send<=15,"%s: Балансировка IP трафика в %s приемлемая т.к. отклонение равно или превысило 15%% и равно - %d%% тест закончился на этапе 4"%(DUT.hostname,int4_name,delta4_send))
